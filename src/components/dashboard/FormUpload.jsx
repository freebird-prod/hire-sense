  ...
  const handleExtractSkills = async () => {
    ...
    try {
      ...
      const res = await axios.post(
        import.meta.env.VITE_FLASK_URL + "/extract_skills",
        formData
      );
      const data = res.data;
      ...
      setSkills(skills);
      toast.success(
        `Extracted ${skills.length} skill(s) for ${name || "Unnamed Candidate"}.`
      );
    } catch (e) {
      console.error(e);
...
`;
      ...
      toast.error(
        "Hire Sense AI analysis failed. Check the console for details.",
        {
          position: "top-right",
        }
      );
      ...
      {file && (
        <div className="mt-6 flex flex-col sm:flex-row gap-4 justify-center">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className={`px-6 py-2 rounded-md text-white font-semibold flex items-center gap-2 ${
              analyzing ? "bg-green-400" : "bg-green-600 hover:bg-green-500"
            }`}
          >
            {analyzing ? (
              <Loader2 className="animate-spin w-4 h-4" />
